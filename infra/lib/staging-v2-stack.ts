...
export class TriviaMapStagingV2Stack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    const basicAuthorization = new cdk.CfnParameter(this, 'BasicAuthorization', { type: 'String', noEcho: true, description: 'Expected Basic authorization header from triviamap/stg/access.' });
    ...
    const basic = new cloudfront.Function(this, 'BasicAuth', { code: cloudfront.FunctionCode.fromInline(`function handler(event) { var request = event.request; if (request.headers.authorization && request.headers.authorization.value === '${basicAuthorization.valueAsString}') return request; return { statusCode: 401, statusDescription: 'Unauthorized', headers: { 'www-authenticate': { value: 'Basic realm="TriviaMap staging"' } } }; }`), runtime: cloudfront.FunctionRuntime.JS_2_0 });
