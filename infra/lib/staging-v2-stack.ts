...
export class TriviaMapStagingV2Stack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    const basic = new cloudfront.Function(this, 'BasicAuth', { code: cloudfront.FunctionCode.fromInline(`var crypto = require('crypto'); function handler(event) { var request = event.request; var value = request.headers.authorization && request.headers.authorization.value; if (!value || crypto.createHash('sha256').update(value).digest('hex') !== '${basicAuthorizationHash.valueAsString}') return { statusCode: 401, statusDescription: 'Unauthorized', headers: { 'www-authenticate': { value: 'Basic realm="TriviaMap staging"' } } }; request.headers['x-triviamap-viewer-ip'] = { value: event.viewer.ip }; if (request.uri.indexOf('/api/') === 0) request.uri = request.uri.substring(4); else if (request.uri.indexOf('/images/') === 0) request.uri = request.uri.substring(7); else if (request.uri.indexOf('.') === -1) request.uri = '/index.html'; return request; }`), runtime: cloudfront.FunctionRuntime.JS_2_0 });
    const imageNotFound = new cloudfront.Function(this, 'ImageNotFound', { code: cloudfront.FunctionCode.fromInline(`function handler(event) { var response = event.response; if (response.statusCode === 403) { response.statusCode = 404; response.statusDescription = 'Not Found'; } return response; }`), runtime: cloudfront.FunctionRuntime.JS_2_0 });
