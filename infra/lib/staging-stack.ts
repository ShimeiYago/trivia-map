...
export class TriviaMapStagingStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    const api = new NodejsFunction(this, 'Api', { runtime: lambda.Runtime.NODEJS_24_X, architecture: lambda.Architecture.ARM_64, entry: path.join(__dirname, '../../apps/api/src/handler.ts'), handler: 'handler', timeout: Duration.seconds(30), memorySize: 512, environment: { IMAGE_BUCKET: images.bucketName, BACKEND_SECRET_ARN: secret.secretArn, FRONTEND_ORIGIN: `https://${distribution.distributionDomainName}`, ...Object.fromEntries(tables.map((table) => [`TABLE_${table.node.id.toUpperCase()}`, table.tableName])) }, bundling: { minify: true, sourceMap: true } });
