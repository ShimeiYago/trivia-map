...
export const transform = (entity: Entity, row: Record<string, unknown>): RecordItem => {
  ...
  switch (entity) {
    case 'Users': return { id: str(row.id), entity: 'User', userId: str(row.id), email: str(row.email).toLowerCase(), username: str(row.username), nickname: str(row.nickname), password: str(row.password), isActive: Boolean(row.is_active), isStaff: Boolean(row.is_staff), isSuperuser: Boolean(row.is_superuser), icon: row.icon || null, socialIcon: row.socialIcon || null, url: row.url || null, dateJoined: iso(row.date_joined), lastLogin: iso(row.last_login), migratedAt: at };
