...
進捗は [stg-manual-test-tracker.xlsx](stg-manual-test-tracker.xlsx) で管理する。`進捗トラッカー` シートの `状態` 列をクリックし、`未着手`、`実施中`、`PASS`、`FAIL`、`BLOCKED` から選択する。各行の `詳細を開く` から `テストケース` シートの目的・事前条件・詳細手順・期待結果・PASS条件・FAIL時の証跡を確認できる。FAIL のときだけ `証跡 / メモ` に詳細を記入する。
